...
function ajouterVote($titre, $type, $date_debut, $date_fin)
{
    $db = connectionDB();
    
    $request = $db->prepare('INSERT INTO vote(titre, type, date_debut, date_fin) VALUES (:titre, :type, :date_debut, :date_fin)');
    $request->execute(array(':titre' => $titre,':type'=> $type, ':date_debut' => $date_debut, ':date_fin'=>$date_fin));
}

function ajouterReponse($id_vote, $reponse, $nb_vote, $visible)
{
    $db = connectionDB();
    
    $request = $db->prepare('INSERT INTO reponse(id_vote, nom_rep, nombre_vote, visible) VALUES (:id_vote, :nom_rep, :nombre_vote, :visible)');
    $request->execute(array(':id_vote' => $id_vote,':nom_rep'=> $reponse, ':nombre_vote' => $nb_vote, ':visible'=> $visible));
}
...
function getIdVote($titre)
{
    ...
    $request = $db->prepare('SELECT id_vote AS num_vote
                                            FROM vote
                                            WHERE titre = :titre');
    $request->execute(array(':titre' => $titre));
    $num_vote = $request->fetch(PDO::FETCH_ASSOC);
